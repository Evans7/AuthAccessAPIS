...
Query for count of users with different roles:-
  rolename="admin"/"user"
    q = db.session.query(User, Role, UserRoles).filter(User.id == UserRoles.user_id,Role.id == UserRoles.role_id,Role.name == rolename).count()
